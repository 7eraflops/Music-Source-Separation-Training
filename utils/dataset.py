...
class MSSDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.latents_path:
            ...
            latent_files = glob(
                os.path.join(self.latents_path, "**", "*.pt"), recursive=True
            )
            for lf in latent_files:
                # Assuming latent file name is UUID.pt
                name = os.path.splitext(os.path.basename(lf))[0]
                # For internal fusion, we might have multiple sources in subdirs
                ...
                if source_name:
                    if isinstance(self.latent_map[name], dict):
                        ...
                        self.latent_map[name] = {"bs_roformer": old_path}
                else:
                    # No subdirectory structure, single source (legacy)
                    if (
                        isinstance(self.latent_map[name], dict)
                        and len(self.latent_map[name]) == 0
                    ):
                        self.latent_map[name] = lf
                    elif not isinstance(self.latent_map[name], dict):
                        pass  # Already set
                    else:
                        # Has subdirs but this file is in root? Use as default
                        self.latent_map[name]["default"] = lf
            if self.verbose and (not dist.is_initialized() or dist.get_rank() == 0):
                # Count total unique tracks
                print(f"Found {len(self.latent_map)} tracks with latents.")
                # Count by source
                source_counts = {}
                for uuid, paths in self.latent_map.items():
    ...
    def _load_latent_chunk(self, track_path, offset, chunk_size):
        ...
        latent_files = self.latent_map[uuid]

        # Handle both dict (multiple sources) and single path (legacy)
        if not isinstance(latent_files, dict):
            latent_files = {"bs_roformer": latent_files}
        ...
        for source_name, latent_file in latent_files.items():
            try:
                ...
                # Handle Dictionary Latents (HTDemucs, SCNet)
                    ...
                    # For SCNet or other models that save as dict
                    # This needs to be implemented based on their format
                    pass
    ...
    def get_metadata(self):
        ...
        if self.dataset_type in [1, 4, 5]:  # Added type 5
            ...
            if should_print:
                print(
                    "Unknown dataset type: {}. Must be 1, 2, 3, 4 or 5".format(
                        self.dataset_type
                    )
                )
            exit()
    ...
    def load_aligned_data(self):
        ...
        latents = self._load_latent_chunk(
            track_path, common_offset if common_offset else 0, self.chunk_size
        )
